...
- **Freshness**: Give every time-sensitive value (prices, availability, measured performance, rankings, statistics, schedules, current versions) a date in brackets straight after the figure. Use the date the source states, e.g. "$4,349 (Beelink store, July 2026)". If the source states no date but you fetched it this run, use the fetch date, e.g. "$4,349 (Beelink store, fetched today)" — a live vendor page IS dated by the fetch and must NOT be called undated. Only use "(undated)" for a figure whose source neither states a date nor was fetched this run. Add "may be outdated" to anything whose stated date is more than 3 months before today. Where two figures for the same metric differ, check their dates first — differently-dated measurements are usually an update, not a contradiction, so give both with their dates.
...
9. **Dating of time-sensitive values**: Every price, benchmark figure, availability claim, ranking, and version number in the report MUST carry a date marker in brackets. Flag each one that has none. Also flag: a figure marked "(undated)" whose source file IS in the workspace (it was fetched this run, so its date is the fetch date, not undated); and any figure whose stated date is more than 3 months before today but is not labelled "may be outdated". Derived values (price-per-GB, differences, totals) inherit the date of the figures they came from and must carry it too.
